...
class Config {
    static long userID = Long.parseLong(PropUtil.getProp("userID")); //牛牛号
    static long trdAcc = 123456L; //业务账号，每个市场都有独立的业务账号，可以通过getAccList获取到。
    static String unlockTradePwdMd5 = MD5Util.calcMD5(PropUtil.getProp("unlockTradePwdMd5"));  //解锁交易密码的md5
    static TrdCommon.SecurityFirm securityFirm = TrdCommon.SecurityFirm.SecurityFirm_FutuSecurities; //trdAcc所属券商
    static String opendIP = PropUtil.getProp("opendIP");
    static short opendPort = Short.parseShort(PropUtil.getProp("opendPort"));
