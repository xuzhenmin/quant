...
public class DemoBase implements FTSPI_Conn, FTSPI_Qot, FTSPI_Trd {
    ...
    QotGetSecuritySnapshot.Response getSecuritySnapshotSync(List<QotCommon.Security> secList)
        ...
        synchronized (syncEvent) {
            synchronized (qotLock) {
                if (qotConnStatus != ConnStatus.READY) { return null; }
        ...
        synchronized (syncEvent) {
            synchronized (qotLock) {
                ...
                QotSub.C2S c2s = QotSub.C2S.newBuilder()
                    .addAllSecurityList(secList)
                    .addAllSubTypeList(subTypeList.stream().mapToInt((QotCommon.SubType subType) -> subType.getNumber())
    ...
    TrdPlaceOrder.Response placeOrderSync(TrdPlaceOrder.C2S c2s) throws InterruptedException {
        ...
        synchronized (syncEvent) {
            synchronized (trdLock) {
                if (trdConnStatus != ConnStatus.READY) { return null; }
    ...
    TrdGetOrderFillList.Response getOrderFillListSync(long accID, TrdCommon.TrdMarket trdMarket,
    ...
    TrdGetHistoryOrderList.Response getHistoryOrderListSync(long accID, TrdCommon.TrdMarket trdMarket,
                                                            TrdCommon.TrdEnv trdEnv,
                                                            TrdCommon.TrdFilterConditions filterConditions,
                                                            List<TrdCommon.OrderStatus> filterStatusList)
        throws InterruptedException {
    ...
    TrdGetHistoryOrderFillList.Response getHistoryOrderFillListSync(long accID, TrdCommon.TrdMarket trdMarket,
                                                                    TrdCommon.TrdEnv trdEnv,
                                                                    TrdCommon.TrdFilterConditions filterConditions)
        throws InterruptedException {
        ReqInfo reqInfo = null;
        ...
        synchronized (syncEvent) {
            synchronized (trdLock) {
                ...
                TrdGetHistoryOrderFillList.Request req = TrdGetHistoryOrderFillList.Request.newBuilder().setC2S(c2s)
                    .build();
